<?php
    ...
    // TODO
    public function CheckFields(array $array){
      // Small sanity tests to avoid some request failures.
      if(!isset($array["orderId"]) && !isset($array["autoGenerateOrderId"]))
      {
        $array["autoGenerateOrderId"] = TRUE;
      }
      if(!isset($array["orderIdIsUnique"]))
      {
        $array["orderIdIsUnique"] = TRUE;
      }
    }
	...
	*************************************************************************************/
    ...
    protected function performRequest($data, $apiRequest, $callBackSuccess = NULL, $callBackFailure = NULL){
        ...
        try{
          ...
          // DEBUG
          echo "SENDING JSON DATA: " . $jsondata . "<br/>\n";
...
// DEBUG ONLY
// RETURN DATA FROM A STANDARD PAGE LOAD TO CHECK CODE IS WORKING
/*
if($_SERVER["REQUEST_METHOD"] == "GET")
{
  $data = array("Message" => "Nothing to process.");
